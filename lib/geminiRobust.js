const ALLOWED_MODELS = ['gemini-2.5-flash', 'gemini-2.0-flash', 'gemini-1.5-flash'];
...
function modelFallbackChain(preferred, options = {}) {
  ...
  if (freeTier) {
    const chain = ['gemini-1.5-flash', 'gemini-2.0-flash'];
    return chain.filter((m, i, a) => a.indexOf(m) === i);
  }
  const tail = ['gemini-2.0-flash', 'gemini-1.5-flash'].filter((m) => m !== p);
  return [p, ...tail];
}
