...
function buildStudioTextPrompt({ topic, platform, tone, profile, intent }) {
  const t = String(topic || '').trim();
  const pl = String(platform || 'Telegram');
  const tn = String(tone || 'вирусный');
  ...
  let prompt = `Ты эксперт по вирусному контенту для ${pl}. Тон: ${tn}. Создай вирусный пост на тему: ${t}. ${intentLine} ${simpleTopicLine} Добавь 3-5 эмодзи. Закончи призывом к действию. Максимум 1000 символов.${ps}`;
