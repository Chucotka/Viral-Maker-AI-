 ...
 */
const { attachmentContextSuffix, imageReferencePromptSuffix } = require('./studioAttachments');
...
function buildStudioTextPrompt({
  ...
  if (normalizedIntent === 'surprise' || isSurpriseRequest(t)) {
    return (
      buildSurpriseStudioPrompt({ platform: pl, tone: tn, profile, recentHistory, risk, goal: inferredGoal }) +
      attachmentContextSuffix(attachments)
    );
  }
...
Ты эксперт по продвижению в VK Видео. Создай структуру для видео на тему: ${t}. Тон: ${tn}.
  ...
  return prompt + attachmentContextSuffix(attachments);
}
...
function buildImageModelPrompt({ prompt, aspectRatio, style, profile, recentHistory, hasReference }) {
  ...
  if (hasReference) text += imageReferencePromptSuffix();
