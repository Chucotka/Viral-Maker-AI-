...
const IMAGE_MODEL_CHAIN = ['gemini-2.5-flash-image', 'gemini-3.1-flash-image'];
...
async function callGenerateContent(apiKey, model, prompt, aspectRatio) {
...
async function generateGeminiImage(opts) {
  ...
  for (const model of IMAGE_MODEL_CHAIN) {
    for (let attempt = 0; attempt < 4; attempt++) {
      try {
        const { mimeType, dataBase64 } = await callGenerateContent(apiKey, model, prompt, aspectRatio);
