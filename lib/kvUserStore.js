...
const { isGuestUserId, guestFreeGenerationLimit } = require('./webGuest');
...
function normalizeUserRecord(raw) {
  ...
  return {
    ...
    lastSeen: raw.lastSeen && typeof raw.lastSeen === 'string' ? raw.lastSeen : null,
  };
}
...
async function getUserRecordsBatch(userIds) {
  ...
  return userIds.map((userId, index) => ({
    userId,
    rec: normalizeUserRecord(raws[index]),
  }));
}
...
async function listActivePaidUsers(opts = {}) {
  ...
  } while (cursor !== '0' && items.length < max && iterations < maxIterations);
...
function parseLastSeenMs(iso) {
  if (!iso || typeof iso !== 'string') return NaN;
  const ms = new Date(iso).getTime();
  return Number.isFinite(ms) ? ms : NaN;
}

/** Не пишем lastSeen чаще раза в 15 мин — достаточно для admin-аналитики. */
const LAST_SEEN_WRITE_INTERVAL_MS = 15 * 60 * 1000;

async function touchUserLastSeen(userId, rec) {
  const now = Date.now();
  const prev = parseLastSeenMs(rec.lastSeen);
  if (Number.isFinite(prev) && now - prev < LAST_SEEN_WRITE_INTERVAL_MS) {
    return rec;
  }
  const next = { ...rec, lastSeen: new Date(now).toISOString() };
  await saveUserRecord(userId, next);
  return next;
}

function countActiveSince(rec, sinceMs) {
  const seen = parseLastSeenMs(rec.lastSeen);
...
async function listSubscriptionOverview(opts = {}) {
  ...
  const now = Date.now();
  const since7d = now - 7 * 864e5;
  const since30d = now - 30 * 864e5;
  let cursor = '0';
  let iterations = 0;
  const maxIterations = 100;

  async function processBatch(userIds) {
    const rows = await getUserRecordsBatch(userIds);
    for (const { userId, rec: initialRec } of rows) {
      let rec = initialRec;
      rec = await expirePlanIfNeeded(userId, rec);
      totalUsers += 1;
      if (countActiveSince(rec, since7d)) activeLast7Days += 1;
      if (countActiveSince(rec, since30d)) activeLast30Days += 1;
  ...
  do {
    ...
    for (let i = 0; i < userIds.length; i += 40) {
      await processBatch(userIds.slice(i, i + 40));
    }
...
/** Бесплатный пул на тарифе Free — без ежедневного сброса. */
const FREE_GENERATION_LIMIT = 5;
const FREE_DAILY_LIMIT = FREE_GENERATION_LIMIT;

function freeGenerationLimitForUser(userId) {
  if (isGuestUserId(userId)) return guestFreeGenerationLimit();
  return FREE_GENERATION_LIMIT;
}

/** Эффективная квота: free-пул + бонусные реферальные генерации. */
function getEffectiveQuota(rec, limit = FREE_GENERATION_LIMIT) {
  if (limit === Infinity) {
    return {
      canGenerate: true,
...
/** Актуальное состояние квоты (free-пул не обновляется по дням). */
async function getQuotaState(userId) {
  let rec = await getUserRecord(userId);
  rec = await expirePlanIfNeeded(userId, rec);
  rec = await touchUserLastSeen(userId, rec);
  const limit = isPaidTierActive(rec) ? Infinity : freeGenerationLimitForUser(userId);
