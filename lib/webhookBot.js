...
const { getPendingImageSave, clearPendingImageSave } = require('./pendingImageSave');
const { sendImageTokenToUser } = require('./sendImageToUser');
...
function registerWebhookHandlers(bot, webAppUrl) {
  ...
  bot.command('start', async (ctx) => {
    ...
    if (payload === 'support') {
      await ctx.reply(
        '💬 Поддержка Viral Maker AI\n\n' +
          'Опишите вопрос одним сообщением — ответим в этом чате.\n\n' +
          'Тарифы, оплата, сохранение картинок, канал, баги — всё сюда.',
        {
          reply_markup: {
            inline_keyboard: [[{ text: '⚡ Открыть студию', web_app: { url: webAppUrl } }]],
          },
        },
      );
      return;
    ...
    if (userId && (payload === 'save_image' || payload === 'save')) {
      const pendingToken = await getPendingImageSave(userId);
      if (pendingToken) {
        const sent = await sendImageTokenToUser(userId, pendingToken);
        if (sent.ok) {
          await clearPendingImageSave(userId);
          await ctx.reply(
            '✅ Картинка выше.\n\nУдержите её → «Сохранить в галерею» / «Save to Photos».\n' +
              'Для файла-документа нажмите на вложение → «Скачать».',
            {
              reply_markup: {
                inline_keyboard: [[{ text: '⚡ Вернуться в студию', web_app: { url: webAppUrl } }]],
              },
            },
          );
          return;
        }
        if (sent.error === 'download_not_found') {
          await clearPendingImageSave(userId);
        }
      }
      await ctx.reply(
        'Нет готовой картинки для сохранения.\n\nСгенерируйте изображение в студии и снова нажмите «Сохранить в галерею».',
        {
          reply_markup: {
    ...
    const startAppUrl = refMatch ? `${webAppUrl}?startapp=${encodeURIComponent(payload)}` : webAppUrl;
    const text = refMatch
      ? '⚡ Тебя пригласили в Viral Maker AI! Открой студию — бонус уже ждёт твоего друга.'
      : '🚀 Бот работает! Открывай студию:';
    await ctx.reply(text, {
      reply_markup: {
        inline_keyboard: [[{ text: '⚡ Открыть Viral Maker AI', web_app: { url: startAppUrl } }]],
      },
    });
  });
