...
function registerWebhookHandlers(bot, webAppUrl) {
  ...
  bot.on('message').filter((ctx) => Boolean(ctx.message?.successful_payment), async (ctx) => {
    if (!isKvConfigured()) {
      ...
      await ctx.reply('Оплата получена, но сервер не сохранил подписку (нет KV). Напишите в поддержку.');
      return;
    }
    ...
    if (!m) {
      ...
      await ctx.reply('Ошибка активации счёта. Обратитесь в поддержку.');
      return;
    }
    const [, plan, userId] = m;
    ...
    if (!payerId || payerId !== String(userId)) {
      ...
      await ctx.reply('Ошибка привязки оплаты к аккаунту. Обратитесь в поддержку.');
      return;
    }
    ...
    await ctx.reply(`✨ Подписка ${plan.toUpperCase()} активирована на ${planDurationDays()} дн. Откройте мини-приложение снова.`);
  });
}
