...
const { buildGeminiPartsFromAttachments } = require('./studioAttachments');
const { createGenerationBudget, isServerlessRuntime, paidMaxCandidates } = require('./generationBudget');
...
function withAttachmentGeminiOpts(prompt, opts = {}) {
  const parts = buildGeminiPartsFromAttachments(prompt, opts.attachments);
...
async function criticReviewText(genAI, modelChain, prompt, opts = {}) {
  try {
    const result = await generateContentRobust(genAI, modelChain, prompt, withAttachmentGeminiOpts(prompt, opts));
...
function logGenTiming(payload) {
  console.log(JSON.stringify({ event: 'text_generation', ...payload, ts: Date.now() }));
}

/** Free: один вызов Gemini (~10–25 с). */
async function generateFastTextContent(genAI, modelChain, prompt, opts = {}) {
  const t0 = Date.now();
  const goal =
    normalizeGoal(opts.goal) === 'auto' ? inferPostGoal(opts) : normalizeGoal(opts.goal);
  const result = await generateContentRobust(genAI, modelChain, prompt, withAttachmentGeminiOpts(prompt, {
    ...opts,
    maxAttemptsPerModel: 3,
  }));
  const metrics = rankCandidate(result.content, opts.recentHistory, []);
  const viralScore = metrics.viralScore;
  const ms = Date.now() - t0;
  logGenTiming({ pipeline: 'fast', plan: opts.plan || 'free', ms, model: result.modelUsed });
  return {
    content: result.content,
    modelUsed: result.modelUsed,
    viralScore,
    draftScore: viralScore,
    rewriteApplied: false,
    selectedAngle: 'fast',
    candidateCount: 1,
    candidateSummary: [],
    criticScore: 0,
    criticNeedsRewrite: false,
    criticVerdict: '',
    criticIssues: [],
    criticStrengths: [],
    criticBreakdown: {},
...
async function generateOneCandidate(genAI, modelChain, prompt, angle, index, total, opts, baseCandidates) {
  try {
    const candidatePrompt = buildCandidatePrompt(prompt, angle, index, total);
    const result = await generateContentRobust(
      genAI,
      modelChain,
      candidatePrompt,
      withAttachmentGeminiOpts(candidatePrompt, opts),
    );
    const metrics = rankCandidate(result.content, opts.recentHistory, baseCandidates);
    return { ...result, ...metrics, angle: angle.id };
  } catch {
    return null;
  }
}

function assembleBestTextResult({
  selected,
  draftSelected,
  candidates,
  critic,
  ...
  const generationMs = Date.now() - tStart;
  logGenTiming({
    pipeline: 'full',
    stage: 'done',
    plan: opts.plan,
    ...
    remainingMs: opts.budget ? opts.budget.remaining() : null,
  });
  return {
    ...
    goal,
    pipelineMode: opts.plan === 'premium' ? 'premium' : 'pro',
    generationMs,
  };
}
...
async function generateBestTextContent(genAI, modelChain, prompt, opts = {}) {
  ...
  const budget = opts.budget || createGenerationBudget();
  const geminiOpts = withAttachmentGeminiOpts(prompt, {
    ...opts,
    maxAttemptsPerModel: opts.maxAttemptsPerModel ?? (isServerlessRuntime() ? 2 : 4),
  });
  const goal = normalizeGoal(opts.goal) === 'auto'
    ? inferPostGoal(opts)
    : normalizeGoal(opts.goal);
  const maxCandidates = Math.max(1, Math.min(3, Number(opts.maxCandidates) || 3));
  const parallel = !!opts.parallelCandidates;
  const angles = candidateAnglesForIntent(opts.intent, goal).slice(0, maxCandidates);
  const candidates = [];
  const baseCandidates = [];

  const tCand0 = Date.now();
  if (parallel && angles.length > 1) {
    const settled = await Promise.all(
      angles.map((angle, i) =>
        generateOneCandidate(genAI, modelChain, prompt, angle, i + 1, angles.length, geminiOpts, []),
      ),
    );
    for (const c of settled) {
      if (c) {
        candidates.push(c);
        baseCandidates.push(c.content);
      }
    }
  } else {
    for (let i = 0; i < angles.length; i += 1) {
      const c = await generateOneCandidate(
        genAI,
        modelChain,
        prompt,
        angles[i],
  ...
  if (!selected) {
    const draftResult = await generateContentRobust(
      genAI,
      modelChain,
      prompt,
      withAttachmentGeminiOpts(prompt, geminiOpts),
    );
    selected = {
      ...draftResult,
  ...
  if (!budget.canSpend(14000)) {
    logGenTiming({
      pipeline: 'full',
      stage: 'critic_skipped',
      plan: opts.plan,
      ms: 0,
      remainingMs: budget.remaining(),
    });
    return assembleBestTextResult({
  ...
  const tCritic0 = Date.now();
  const critic = await criticReviewText(genAI, modelChain, criticPrompt, geminiOpts);
  logGenTiming({
    pipeline: 'full',
    stage: 'critic',
    plan: opts.plan,
    ms: Date.now() - tCritic0,
    remainingMs: budget.remaining(),
  });

  let rewriteResult = null;
  let rewriteScore = null;
  const wantsRewrite = critic.needsRewrite || critic.score < 82;
  if (wantsRewrite && budget.canSpend(18000)) {
    try {
      ...
      rewriteResult = await generateContentRobust(
        genAI,
        modelChain,
        rewritePrompt,
        withAttachmentGeminiOpts(rewritePrompt, geminiOpts),
      );
...
/**
 * Free — быстрый режим; Pro — 2 угла + критик; Premium — 3 угла параллельно + критик.
 */
async function generateStudioTextContent(genAI, modelChain, prompt, opts = {}) {
  const paid = !!opts.paidTierActive;
  const plan = opts.plan === 'premium' || opts.plan === 'pro' ? opts.plan : 'free';
  if (!paid) {
    return generateFastTextContent(genAI, modelChain, prompt, { ...opts, plan: 'free' });
  }
  const budget = createGenerationBudget();
  const paidOpts = { ...opts, budget };
  if (plan === 'premium') {
    return generateBestTextContent(genAI, modelChain, prompt, {
      ...paidOpts,
      plan: 'premium',
      maxCandidates: paidMaxCandidates('premium', 3),
  ...
  return generateBestTextContent(genAI, modelChain, prompt, {
    ...paidOpts,
    plan: 'pro',
    maxCandidates: paidMaxCandidates('pro', 2),
    parallelCandidates: false,
  });
}

module.exports = {
  generateStudioTextContent,
  generateFastTextContent,
