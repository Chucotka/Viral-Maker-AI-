...
async function generateBestTextContent(genAI, modelChain, prompt, opts = {}) {
  ...
  if (parallel && angles.length > 1) {
    const settled = await Promise.all(
      angles.map((angle, i) => generateOneCandidate(genAI, modelChain, prompt, angle, i + 1, angles.length, opts, [])),
    );
  ...
  logGenTiming({ pipeline: 'full', stage: 'critic', plan: opts.plan, ms: Date.now() - tCritic0 });
