...
async function generateFastTextContent(genAI, modelChain, prompt, opts = {}) {
  ...
  const result = await generateContentRobust(genAI, modelChain, prompt, {
    ...opts,
...
async function generateBestTextContent(genAI, modelChain, prompt, opts = {}) {
  ...
  if (!selected) {
    const draftResult = await generateContentRobust(genAI, modelChain, prompt, geminiOpts);
    selected = {
      ...draftResult,
  ...
  if (wantsRewrite && budget.canSpend(18000)) {
    try {
      ...
      rewriteResult = await generateContentRobust(genAI, modelChain, rewritePrompt, geminiOpts);
