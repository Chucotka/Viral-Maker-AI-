...
const ADMIN_DENIAL_MESSAGES = {
  guest_session:
    'Вы вошли как гость в браузере. Admin-доступ только после входа через Telegram (кнопка «Войти через Telegram в браузере» или Mini App).',
  not_owner:
...
function getAdminAccessStatus(req, userId, authMeta = {}) {
  ...
  // Mini App: initData уже подписан Telegram — дополнительный секрет не нужен.
  if (source === 'telegram_mini_app') {
    return { ok: true, userId: id, via: 'telegram_mini_app' };
  }
