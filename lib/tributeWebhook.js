...
async function handleTributeWebhook(req, res) {
  ...
  if (req.method === 'GET' || req.method === 'HEAD') {
    return res.status(200).json({ ok: true, endpoint: 'tribute-webhook' });
  }
  ...
  if (name === 'test_event' || body.test_event) {
    console.info('Tribute webhook: test_event ok');
  ...
  return res.json({ status: 'ok', activated: true, userId, plan: plan || 'pro' });
}
