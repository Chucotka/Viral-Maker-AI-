...
const { resolveWebAuthOrigin } = require('./webOrigin');
...
function buildReferralLink(userId, opts = {}) {
  ...
  const siteOrigin = String(opts.webAppUrl || resolveWebAuthOrigin()).replace(/\/+$/, '');
  const param = `${REFERRAL_PREFIX}${userId}`;
  return {
    param,
    telegramLink: `https://t.me/${botUsername}?startapp=${encodeURIComponent(param)}`,
    webLink: `${siteOrigin}/?startapp=${encodeURIComponent(param)}`,
  };
}
